        ...
        <thead>
          <tr className="table-header">
            {headerContent.map((ele) => (
              <th className="header-data" key={ele}>{ele}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {Content.map((data, index) => (
            <tr className="body-row" key={data._id}>
              {location.pathname === '/GetBlog' ? (
                ...
                <>
                  <td className="body-data">{data._id}</td>
                  <td className="body-data">{data.orderState}</td>
                  <td className="body-data">
                    <select onChange={(event)=>handleOrderState(event,data._id) } className="select-order-state">
                      <option disabled selected >Choose Order State</option>
                      <option>Pending</option>
                      <option>Order Placed</option>
                      <option>Order has been shipped</option>
                    </select>
                  </td>
                  <td className="body-data">{data.totalPrice}$</td>
                </>
              ) : (
                <>
                  <td className="body-data">{data.animalType}</td>
